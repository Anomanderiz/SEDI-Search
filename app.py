...
def server(input, output, session):
    ...
    def _do_parse():
        ...
        if not dfile:
            ...
            try: donors = pd.read_csv(io.BytesIO(content))
            except Exception: donors = pd.read_excel(io.BytesIO(content))
            if 'donor_id' not in donors.columns: donors['donor_id'] = donors.index.astype(str)
            if 'aliases' not in donors.columns: donors['aliases'] = ''
        ...
        if not pfile:
            ...
            uf = pfile[0]; pdfbytes = uf.read(); tx = parse_sedi_pdf(pdfbytes)
